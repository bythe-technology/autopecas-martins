...
export function CatalogView({ initialCategory = "Todas", initialQuery = "", vehicle = {} }: { initialCategory?: string; initialQuery?: string; vehicle?: VehicleFilter }) {
  ...
  const products = useMemo(() => catalogProducts.filter((product) => {
    ...
    const matchesStructuredVehicle = !vehicle.make || Boolean(product.fitment
      && product.fitment.make === vehicle.make
      && (!vehicle.model || product.fitment.models.includes(vehicle.model))
      && (!vehicle.year || ((!product.fitment.yearFrom || selectedYear >= product.fitment.yearFrom) && (!product.fitment.yearTo || selectedYear <= product.fitment.yearTo))));
  ...
  }), [partQuery, vehicleQuery, category, vehicle.make, vehicle.model, vehicle.year]);
