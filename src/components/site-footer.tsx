...
import { Clock, MessageCircle, Pin } from "./icons";
...
export function SiteFooter() {
  return <footer className="site-footer">
    <div className="container footer-main">
      <div className="footer-about"><Link href="/" className="footer-brand"><Image src="/images/logo-apm-clean.png" alt="" width={68} height={68} /><span><strong>Auto Peças Martins</strong><small>Peças e serviços automotivos</small></span></Link><p>Atendimento local, estoque diversificado e suporte para encontrar a peça certa para o seu veículo.</p><span className="company-document">AUTO PEÇAS MARTINS LTDA · CNPJ {store.cnpj}</span></div>
      ...
      <div className="footer-column"><span className="footer-label">Fale conosco</span><a href={whatsappUrl("Olá! Vim pelo site da Auto Peças Martins.")} target="_blank" rel="noreferrer"><MessageCircle size={18} /><span><small>WhatsApp</small>{store.phoneDisplay}</span></a><a href={mapsUrl} target="_blank" rel="noreferrer"><Pin size={18} /><span><small>Visite a loja</small>{store.address}<br />{store.city} · {store.state}</span></a></div>
      <div className="footer-column"><span className="footer-label">Horário</span><div className="footer-hours"><Clock size={18} /><span>{store.hours.map((hour) => <small key={hour}>{hour}</small>)}<small>Feriados: consulte a equipe</small></span></div></div>
    </div>
    ...
    <div className="developer-strip"><a className="bythe-credit" href="https://bythe.tech" target="_blank" rel="noreferrer" aria-label="Site desenvolvido pela BYTHE Technology"><span>DESENVOLVIDO POR</span><strong><i aria-hidden="true">B</i>YTHE</strong></a></div>
  </footer>;
}
